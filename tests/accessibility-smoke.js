...
assert.match(app, /drag\.holdTimer = setTimeout\([\s\S]*?\}, 450\);/);
...
assert.match(app, /specials-order-drag-handle/);
assert.doesNotMatch(app, /role: "listitem",\s*draggable:/);
assert.match(css, /\.specials-group-order-panel \.specials-order-drag-handle \{[\s\S]*?touch-action: pan-y !important;/);
